...
global Mstate screenPTR screenNum screenPTROff loopTrial
...
stimSizecm=2*Pstruct.radius*2*pi/360*Mstate.screenDist;
stimSize=round(stimSizecm*pixpercmX);


%%%%%%%%%%%%%%%%%%

xran = [Pstruct.x_pos-floor(stimSize/2)+1  Pstruct.x_pos+ceil(stimSize/2)];
yran = [Pstruct.y_pos-floor(stimSize/2)+1  Pstruct.y_pos+ceil(stimSize/2)];
...
Screen(screenPTR, 'Flip');
if loopTrial ~= -1
    digWord = 7;  %toggle 2nd bit to signal stim on
    DaqDOut(daq, 0, digWord);
end
for i = 2:Nstimframes
    Screen('CopyWindow',screenPTROff,screenPTR,[],StimLoc);
