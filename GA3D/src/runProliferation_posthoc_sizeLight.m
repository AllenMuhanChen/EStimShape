function [message,nStim] = runProliferation_posthoc_sizeLight(folderName,gaInfo,postHocId,conn)
    ...
    for linNum=1:2
        ...
        while jj <= nPosthoc*nVariantConds
            ...
            switch mod(jj,nVariantConds)
                case 1;  stim = parentStim; % original; do nothing

                case 2;  stim.shape.lighting = [   0   200    200]; % top
                case 3;  stim.shape.lighting = [   0  -200    200]; % bottom
                case 4;  stim.shape.lighting = [ 200     0    200]; % right
                case 5;  stim.shape.lighting = [-200     0    200]; % left

                case 6;  stim.shape.s = stim.shape.s * 2;
                case 7;  stim.shape.s = stim.shape.s * 3;
                case 0;  stim.shape.s = stim.shape.s * 4;
            end

            switch(positionId)
                case 1;  stim.shape.x = stim.shape.x + 5;
                case 2;  stim.shape.y = stim.shape.y + 5;
