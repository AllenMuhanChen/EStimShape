function plotSurfSta_ver1(runId,binSpec,sta,sta_shuff,surfFitParams,resp,data,is3d)
    [selectResp,~,~,~,thumb] = getTopStim(resp,data,is3d);
    
    if length(sta) == 2
        % sta(1).surf = filterSta(sta(1).surf,binSpec);
        % sta(2).surf = filterSta(sta(2).surf,binSpec);
        
        staMult = sta(1).surf .* sta(2).surf;
    else
        staMult = sta.surf;
    end
    staMult = thresholdSta(staMult);
...
function sta = filterSta(sta,binSpec)
    binCenters = binSpec.surf.binCenters;
    ...
    parfor ii=1:size(x,1)
        if ~mod(ii,size(x,1)/500)
            fprintf('.')
        end
        binsToSmoothOver = zeros(size(x,1),1);
        for jj=1:length(padding)
            if padding(jj) == 's'
                nei = find(sphNeighbours(x(ii,jj),:));
                binsToSmoothOver = binsToSmoothOver + ismember(x(:,jj),nei);
            elseif padding(jj) ~= 'i'
                binsToSmoothOver = binsToSmoothOver + (abs(x(:,jj) - x(ii,jj)) < 2);
            end
        end
        binsToSmoothOver = binsToSmoothOver == length(padding);
        binsToSmoothOver(ii) = 0;
        centralBins(ii) = centralBins(ii) + coeffs(2) * sum(sta(binsToSmoothOver));
    end
    fprintf('\n')
    sta = centralBins;
end