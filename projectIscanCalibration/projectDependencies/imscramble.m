...
function [imScrambled,RandomPhase] = imscramble(im,p,rescale,RandomPhase)
...
if nargin < 4
    RandomPhase = angle(fft2(rand(imSize(1), imSize(2)))); %generate random phase structure in range p (between 0 and 1)
...
for layer = 1:imSize(3)
    ...
    imScrambled(:,:,layer) = ifft2(Amp(:,:,layer).*exp(1i*(Phase(:,:,layer))));
end
