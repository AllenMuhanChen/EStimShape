...
def smooth_matrices(matrices: list[LabelledMatrix], sigma=20) -> list[LabelledMatrix]:
    smoothed_matrices = []
    for matrix in matrices:
        smoothed_matrix = gaussian_filter(matrix.values, sigma)
        smoothed_matrices.append(LabelledMatrix(matrix.axes, smoothed_matrix))

    return smoothed_matrices


def calculate_response_weighted_average(matrices: list[LabelledMatrix], response_vector: list[float]) -> list[LabelledMatrix]:
    ...
    for stim_index, matrix in enumerate(matrices):
        ...
        response_weighted_matrices.append(LabelledMatrix(matrix.axes, response_weighted_matrix))
    ...
    return LabelledMatrix(matrices[0].axes, response_weighted_average)


def normalize_matrix(matrix):
    max_val = np.amax(matrix.values)
    normalized_matrix = np.divide(matrix.values, max_val)
    return LabelledMatrix(matrix.axes, normalized_matrix)
