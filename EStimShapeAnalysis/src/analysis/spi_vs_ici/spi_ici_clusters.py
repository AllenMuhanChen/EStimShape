...
def create_frequency_preference_scatter_plots():
    ...
    # Query solid preference indices but only for cluster channels - NOW INCLUDING P-VALUE
    solid_query = """
    ...
    solid_df = pd.DataFrame(solid_data, columns=['session_id', 'unit_name', 'solid_preference_index', 'p_value'])
    ...
    for session in sessions:
        ...
        sig_units = session_data[(pd.notna(session_data['p_value'])) & (session_data['p_value'] < 0.05)][
            'unit_name'].nunique()
        print(f"  {session}: {unique_units} unique units ({sig_units} sig.), {total_rows} total rows")
    ...
    for frequency, stats in frequency_stats.items():
        ...
        print(f"  Units: {stats['n']} ({stats['n_significant']} significant)")
...
def plot_frequency_data(merged_df, frequency, sessions):
    ...
    if freq_data.empty:
        ...
        return {'n': 0, 'n_significant': 0, 'r_squared': 0, 'r_value': 0, 'p_value': 1, 'slope': 0, 'intercept': 0}
        ...
        if not session_data.empty:
            ...
            if sig_mask.any():
                plt.scatter(session_data[sig_mask]['solid_preference_index'],
                            ...
                            label=f'Session {session_id} (n={unique_units}, {sig_units} sig.)')
            ...
            if (~sig_mask).any():
                ...
                if not sig_mask.any():
                    plt.scatter([], [], c=[session_colors[i]], alpha=0.7, s=60,
                                label=f'Session {session_id} (n={unique_units}, 0 sig.)')
