...
-------------------------
Orientation-tuning view built on the spi_vs_ici machinery.

``plot_orientation_strong_spi_vs_ici`` (and its pooled variant) - the usual
Solid-vs-Isochromatic preference scatter (by frequency), but restricted to the
cells *and* frequencies where orientation tuning is strong: a (cell, frequency)
pair is kept only if its orientation tuning depth (``max_minus_min`` from
``PreferredOrientations``) is at least ``threshold`` (default 0.7 = 70%) of that
cell's peak tuning depth across all of its frequencies. This mirrors the "strong
frequencies" idea in spi_vs_ici_preferred_freq.parse_strong_frequencies, applied
to orientation tuning instead of raw response.

The cycles-per-RF vs orientation-tuning plot lives separately in
``receptive_fields/rf_size_vs_orientation_tuning.py``.

Not every session has orientation data; a missing/empty ``PreferredOrientations``
table is handled gracefully (empty DataFrame, a printed warning, no crash).
"""
...
if __name__ == "__main__":
    # Channel selection mode (see spi_vs_ici.py for options).
    ...
    # SPI vs ICI for orientation-strong cells/frequencies (tuning >= threshold of peak)
    ...
    # Same data, pooled into a single scatter (not split by frequency)
    plot_orientation_strong_spi_vs_ici_combined(
        selection_mode=selection_mode, threshold=threshold,
        regression_method='ols', spi_regression_max=0.3,
        save_path=_path("orientation_strong_spi_vs_ici_combined.png"))

    # The cycles-per-RF vs orientation-tuning plot now lives in
    # receptive_fields/rf_size_vs_orientation_tuning.py.
