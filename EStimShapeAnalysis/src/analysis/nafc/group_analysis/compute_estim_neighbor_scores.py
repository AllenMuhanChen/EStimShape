...
def compute_session_neighbor_scores(session_id, *, n_neighbors=3,
    ...
    """Compute {estim_spec_id: {metric_name: {'mean': .., 'worst': ..}}} for one
    session across all metrics."""
...
def _ensure_neighbor_scores_table(repo_conn: Connection):
    ...
    (session, spec, metric, aggregation) so new metrics never need a schema change."""
    repo_conn.execute(
        """
...
def save_session_neighbor_scores(repo_conn, session_id, scores_by_spec):
    """Upsert all metric/aggregation scores for a session."""
    n_rows = 0
            ...
            for aggregation, value in aggs.items():
                repo_conn.execute(
                    ...
                    INSERT INTO EStimNeighborScores
                        (session_id, estim_spec_id, metric_name, aggregation, value)
                    VALUES (%s, %s, %s, %s, %s)
...
def run_for_sessions(start_session_id=None, exclude_session_ids=None, *,
                     ...
                     corr_method='spearman'):
    """Compute and persist all neighbour-similarity metrics across sessions.

    Sessions that raise (missing GA DB, missing vectors, etc.) are reported but
    don't abort the run."""
    repo_conn = Connection("allen_data_repository")
    ...
    for sid in session_ids:
        print(f"\n=== {sid} ===")
        try:
            scores = compute_session_neighbor_scores(
                sid, n_neighbors=n_neighbors, exclude_other_estim=exclude_other_estim,
            ...
            continue
        if scores:
            save_session_neighbor_scores(repo_conn, sid, scores)
