"""Standalone "RF size vs orientation tuning" plot.

Scatter of cycles-per-RF (stimulus frequency x RF diameter) against orientation
tuning depth (``max_minus_min`` from ``PreferredOrientations``), one point per
(unit, frequency). Orientation tuning is z-scored per unit across its frequencies
(so units with different firing rates are comparable), and a binned mean +/- SEM
trend line reveals any preferred number of cycles per RF for orientation tuning.

Mirrors ``plot_cycles_per_rf_vs_response`` in ``rf_size_vs_preferred_freq.py`` and
reuses the RF-radius lookup (with cluster fallback) from
...
def load_orientation_tuning(filter_type='all'):
    """Load per-(unit, frequency) orientation tuning depth from ``PreferredOrientations``.

    Args:
        filter_type: Channel selection (same options as rf_size_vs_preferred_freq.py).
            - 'all': every PreferredOrientations row.
    ...
    if not rows:
        print(f"No PreferredOrientations rows found for filter_type='{filter_type}'")
        return None
...
def expand_orientation_tuning(filter_type='all', normalize='zscore'):
    ...
    normalized across that unit's tested frequencies (per-unit z-score by default),
    and ``cycles_per_rf = frequency * 2 * rf_radius`` is computed.
    ...
    """
    df = load_orientation_tuning(filter_type)
...
def plot_rf_size_vs_orientation_tuning(filter_type='all', normalize='zscore',
                                       n_bins=8, bin_edges=None, xlim=None, save_path=None):
    """Cycles per RF (x) vs orientation tuning (y), with a binned mean +/- SEM trend.

    One point per (unit, frequency), orientation tuning depth (max - min) z-scored per
    unit across its frequencies, points colored by stimulus frequency, with a black
    ...
    """
    long_df = expand_orientation_tuning(filter_type, normalize)
    ...
    print(f"RF-size vs orientation tuning (filter_type='{filter_type}', normalize='{normalize}'): "
    ...
    ax.set_title(f'Orientation tuning vs cycles per RF (filter_type={filter_type})\n'
