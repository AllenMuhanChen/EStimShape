    ...
    def run(self, experiment_id: int, lineages: List[Lineage], gen_id: int):
        ...
        for lineage in lineages:
            for stimulus in lineage.stimuli:
                if stimulus.gen_id == gen_id:
                    if not is_side_test_stimulus(stimulus):
                        self._assign_2d_or_3d(stimulus, lineage)
                        stimulus.lineage = lineage  # create new reference to containing lineage to use later
        ...
        # Now we write the stimuli to the database
        for stimulus in self.top_2d:
            self._write_sidetest_stim_to_db(gen_id, stimulus, SIDETEST_2Dvs3D_Type.THREED_SHADE)
            self._write_sidetest_stim_to_db(gen_id, stimulus, SIDETEST_2Dvs3D_Type.THREED_SPECULAR)

        for stimulus in self.top_3d:
            self._write_sidetest_stim_to_db(gen_id, stimulus, SIDETEST_2Dvs3D_Type.TWOD_HIGH)
            self._write_sidetest_stim_to_db(gen_id, stimulus, SIDETEST_2Dvs3D_Type.TWOD_LOW)

    @staticmethod
    def _write_sidetest_stim_to_db(gen_id, stimulus, mutation_type: SIDETEST_2Dvs3D_Type):
