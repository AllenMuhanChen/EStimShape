...
class MultiGaDbUtil:
    ...
    # ------------------------------------------------------------------
    # MUAChannelResponses: thin delegators to MuaChannelResponseStore
    # (src/pga/mua_channel_responses.py), which owns the table's SQL.
    # ------------------------------------------------------------------
    def mua_store(self, mua_metric: str = None):
        from src.pga.mua_channel_responses import MuaChannelResponseStore, DEFAULT_MUA_METRIC
        return MuaChannelResponseStore(self.conn, mua_metric or DEFAULT_MUA_METRIC,
                                       db_util=self, auto_populate=False)

    def create_mua_channel_responses_table_if_not_exists(self):
        self.mua_store().create_table_if_not_exists()
    ...
    def add_mua_channel_responses_in_batch(self, insert_data):
        ...
        self.mua_store().write_rows(insert_data)

    def read_mua_responses_for(self, stim_id, channel: str, mua_metric: str):
        return self.mua_store(mua_metric).response_vector(stim_id, channel)

    def read_stims_with_no_mua_responses(self, ga_name: str, mua_metric: str):
        """Stims (in the current experiment's lineages) with no MUAChannelResponses
        row carrying spike timestamps for `mua_metric`."""
        current_experiment_id = self.read_current_experiment_id(ga_name)
        lineages_for_experiment_id = self.read_lineage_ids_for_experiment_id(current_experiment_id)
        self.add_catch_lineage(lineages_for_experiment_id)
        return self.mua_store(mua_metric).stims_needing_parse(lineages_for_experiment_id)
