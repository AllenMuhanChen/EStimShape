...
class MockDataLoader(DataLoader):
    ...
    def load_data_for_channels(self, max_gen: int | None = None):
        # Simulate generations by scaling the number of stimuli (features) with max_gen.
        gen = self.max_gen if max_gen is None else max_gen
        n_features = max(2, int(100 * gen / self.max_gen))
        X, _ = make_blobs(n_samples=len(self.channel_mapper.channels_top_to_bottom), centers=8, n_features=n_features, random_state=42, shuffle=False)
