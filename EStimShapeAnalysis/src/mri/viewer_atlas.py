...
from src.mri.correction import load_corrections, save_corrections
...
class AtlasMixin:
    ...
    def _load_atlas_from_path(self, nifti_path):
        ...
        try:
            ...
            # Sidecar JSON alongside the NIfTI — now only stores per-region
            # color highlights (atlas alignment rides on the subject correction).
            self.atlas_corr_json_path = self._atlas_corr_json_for(nifti_path)
            _, self.atlas_corr_config = load_corrections(self.atlas_corr_json_path)
    ...
    def _on_atlas_appearance_change(self, event=None):
        self.atlas_contour_color = self.atlas_color_var.get()
        try:
