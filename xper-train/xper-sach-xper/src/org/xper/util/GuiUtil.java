...
public class GuiUtil {
	public static void makeDisposeOnEscapeKey (final JFrame frame) {
		...
		frame.getRootPane().getActionMap().put(action, action);
		frame.getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(stroke, action);
	}
	
	public static JDialog createProgressDialog(JFrame frame, String message) {
		JDialog progressDialog = new JDialog(frame, "Please wait", false);
		...
		messageLabel.setFont(new Font("System", Font.BOLD, 12));
		...
		padding.setBorder(BorderFactory.createLineBorder(Color.gray));
		
		progressDialog.getContentPane().add(padding, BorderLayout.CENTER);
