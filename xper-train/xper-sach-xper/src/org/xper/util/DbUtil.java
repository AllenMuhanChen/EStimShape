...
public class DbUtil {
	...
	public boolean isBubblesPostHoc() {
		JdbcTemplate jt = new JdbcTemplate(dataSource);
		boolean returnable = false;
		long tstamp = jt.queryForLong("select max(tstamp) from DescriptiveInfo");
		int isBubblesPostHoc = jt.queryForInt("select isBubblesPostHoc from DescriptiveInfo where tstamp = ? ",new Object[]{new Long(tstamp)});
		if (isBubblesPostHoc == 1) returnable = true;
