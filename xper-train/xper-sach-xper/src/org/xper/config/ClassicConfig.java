...
//		console.setCanvasScaleFactor(2.5);
	...
	public DataAcqController dataAcqController() {
		...
		if (isRealExpt()) 
			experimentAcqOffline = false;
		else
			experimentAcqOffline = true;
		controller.setOffline(experimentAcqOffline || acqConfig.acqDriverName.equalsIgnoreCase(acqConfig.DAQ_NONE));
		return controller;
	}
	...
	public AbstractRenderer consoleGLRenderer () {
		...
		renderer.setDepth(xperMonkeyScreenDepth());
		renderer.setHeight(xperMonkeyScreenHeight());
		renderer.setWidth(xperMonkeyScreenWidth()/2);	// TODO: I changed this! -shs (divided by 2, was squashed)
		renderer.setPupilDistance(xperMonkeyPupilDistance());
		return renderer;
	}
...
//		win.setPixelFormat(new PixelFormat(0, 8, 1));
	...
	public boolean isRealExpt() {
		return baseConfig.dbUtil().isRealExpt();
	}
