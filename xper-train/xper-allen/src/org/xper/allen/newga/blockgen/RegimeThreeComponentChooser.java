...
//        }
    ...
    private List<Long> findRegimeTwoFamily(long parentId) {
        ...
        while(!getStimTypeFor(closestRegimeTwoAncestor).equals(stimTypeForRegime.get(Regime.TWO))) {
        ...
        if (hasRegimeTwoAncestor) {
            ...
            for (Long uncle : closestRegimeTwoUncles) {
                if (dbUtil.readStimTypeFor(uncle).equals(stimTypeForRegime.get(Regime.TWO))) {
        ...
        // consider all of its siblings.
            ...
            for (Branch<Long> child : immediateChildren) {
                if (dbUtil.readStimTypeFor(child.getIdentifier()).equals(stimTypeForRegime.get(Regime.TWO))) {
        ...
        if (family.isEmpty()){
            ...
            familyTree.forEach(new Consumer<Branch<Long>>() {
                ...
                public void accept(Branch<Long> branch) {
                    if (dbUtil.readStimTypeFor(branch.getIdentifier()).equals(stimTypeForRegime.get(Regime.TWO))) {
