...
public class DistancedDistractorsUtil {
	...
	public DistancedDistractorsUtil(int numChoices, double lowerRadiusLim, double upperRadiusLim, double distractorDistanceLowerLim, double distractorDistanceUpperLim){
		...
		double distractorDistance = NAFCSpecWriter.inclusiveRandomDouble(distractorDistanceLowerLim, distractorDistanceUpperLim);
		...
		match_coords = NAFCSpecWriter.polarToCart(match_radii, match_angle);
		...
		for (int i=0; i < numChoices-1; i++){
			...
			distractor_coords.add(NAFCSpecWriter.polarToCart(distractor_radii[i], distractor_angles[i])); //polar to cartesian
		}
	}
